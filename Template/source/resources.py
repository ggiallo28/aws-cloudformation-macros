...
class Template(AWSObject):
	...
	def get_value(self, key, template_params, default=None):
	    ...
	    if hasattr(value, 'Ref'):
	    	value = template_params[value['Ref']]
...
def s3_import(request_id, resource_id, resource_obj, template_params, aws_region):
...
def codecommit_import(request_id, resource_name, resource_obj, template_params, aws_region):
    repo = resource_obj.get_value('Repo', template_params)
    branch = resource_obj.get_value('Branch', template_params, default=BRANCH_DEFAULT)
    path = resource_obj.get_value('Path', template_params, default=TEMPLATE_NAME_DEFAULT)
...
def github_import(request_id, resource_name, resource_obj, template_params, aws_region):
    repo = resource_obj.get_value('Repo', template_params)
    branch = resource_obj.get_value('Branch', template_params, default=BRANCH_DEFAULT)
    path = resource_obj.get_value('Path', template_params, default=TEMPLATE_NAME_DEFAULT)
