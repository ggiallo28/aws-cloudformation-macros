...
class TemplateLoader(TemplateGenerator):
        ...
        if type(data) == tuple:
            ...
            if key == "DependsOn":
                for index, _ in enumerate(value):
                    if value[index].replace(self.__macro_name,
                                            "") in self.logical_ids:
                        value[index] = self.__macro_name + self.prefix + value[
                            index].replace(self.__macro_name, "")
    ...
    def set_attrs(self, top_level_resource, import_templates):
        ...
        if hasattr(top_level_resource, 'DependsOn'):
            ...
            values += [
                value for value in top_depends_on
                if not self.__macro_name in value
            ]

            ## Append or set DependsOn value.
            for title in self.resources:
                if hasattr(self.resources[title], 'DependsOn'):
                    bottom_depends_on = getattr(self.resources[title],
                                                'DependsOn')
                    setattr(self.resources[title], 'DependsOn',
                            values + bottom_depends_on)
                else:
    ...
    def resolve_attrs(self, import_templates):
        for title in self.resources:
            if hasattr(self.resources[title], 'DependsOn'):
                ...
                depends_on_template = [
                    d for d in depends_on if self.__macro_name in d
                ]
                depends_on_aws = [
                    d for d in depends_on if self.__macro_name not in d
                ]
